...
## Status Atual
- **Concluído**:
  - Módulo de Louvor & Cifras (Integração Cifra Club): Marcação da flag `is_louvor` no cadastro de momentos de liturgia (`MomentoLiturgiaController.php`, `Views/liturgia/momentos.php`), criação do model `LiturgiaMusica.php`, serviço de raspagem/API `Services/CifraClubService.php` (com fallback nativo PHP), rotas `/escala/musica/*` (`MusicaController.php`), formulário de vinculação de repertório e visualizador responsivo de cifras (`Views/Escala/cifra.php`).
