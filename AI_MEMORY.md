...
## Status Atual
- **Concluído**:
  - Tratamento Global de Erros e Controle via `.env` (`APP_DEBUG`): Adicionada a flag `APP_DEBUG` no `.env` e `.env.example` e o método `SecurityHelper::formatUserErrorMessage()`. Em ambiente de produção (`APP_DEBUG=false`), qualquer erro ou exceção de banco de dados (ex: `PDOException`, SQLSTATE) é capturado e transformado em mensagem amigável ao usuário final sem expor detalhes técnicos.
  - Geração Automática de Escalas com Rotação Semanal: Criado o `EscalaGeneratorService.php` e a rota `/escala/gerar-automatica`. A geração inteligente sorteia voluntários ativos promovendo rotatividade justa sem repetições consecutivas e atribui compulsoriamente momentos de Louvor e Palavra ao Líder Principal.
  - Flag "Líder Principal da Célula" nos Usuários: Adicionado o campo `is_lider_principal` nas tabelas `usuarios` (`db/init.sql` e `db/schema_sqlite.sql`), `Models/Usuario.php`, `Controllers/UsuarioController.php` e nas views `Views/usuarios/create.php` e `edit.php`.
  - Flag "É Palavra" nos Momentos da Liturgia: Adicionada a flag `is_palavra` nas tabelas `momentos_predefinidos` e `momentos_liturgia`, `MomentoLiturgiaController.php` e na view `Views/liturgia/momentos.php`.
  - Remoção Estrita de `ensureSchema()`: Removidas todas as funções `ensureSchema()` e DDLs em tempo de execução dos models PHP, concentrando a definição de esquemas exclusivamente nos scripts de inicialização de banco no boot dos containers.
  ...
  - Correção de Hash de Senhas Iniciais e Garantia de Login: Atualizados os hashes BCrypt válidos para a senha padrão `senha123` dos usuários iniciais (`admin@celula.com` e `joao@celula.com`) em `db/init.sql` e `db/schema_sqlite.sql`, com trava idempotente `ensureDefaultUsers` em `Config/Database.php`.
  - Integração do `.env` com Docker Compose e Centralização de Scripts SQL: O arquivo `.env` passa a fornecer dinamicamente as credenciais de banco de dados (`DB_HOST`, `DB_USER`, `DB_PASS`, `DB_NAME`, `DB_PORT`) para o `docker-compose.yml`, e todos os arquivos `.sql` foram consolidados exclusivamente na pasta `db/` (`db/init.sql` e `db/schema_sqlite.sql`).
  - Correção de Conectividade PostgreSQL e Migração de Coluna `codigo_acesso`: Injeção de variáveis de ambiente no `docker-compose.yml` (`DB_HOST=db`, `DB_USER=root`, `DB_PASS=rootpassword`), retry automático de conexão no `Config/Database.php`, atualização de `db/init.sql` e `db/schema_sqlite.sql` e desacoplamento de `ALTER TABLE` no `Models/Presenca.php` para migração idempotente sem erros de banco.
  - Acesso de Visitantes via Código de Convite: Geração automática de código de acesso único (ex: `V8K2P9`) ao cadastrar visitantes, tela de login dedicada (`/visitante` e `Views/auth/visitante.php`), controlador `VisitanteController.php`, middleware de restrição *Read-Only* por `celula_id` em `public/index.php`, exibição de badge do código e botão de copiar em `Views/Escala/show.php` e testes unitários validados (`tests/VisitanteControllerTest.php`).
