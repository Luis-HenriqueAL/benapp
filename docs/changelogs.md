...
- **Modal de Alerta Estado da Arte (Substituição de `alert()` Nativo)**: Criado o componente global de modal de alerta [globalAlertModal](file:///home/luis/dev/projetos/benApp/Views/layout.php) em [Views/layout.php](file:///home/luis/dev/projetos/benApp/Views/layout.php) e sobrescrita global da função `window.alert()`. Popups padrão do navegador foram 100% eliminados e substituídos por modais elegantes em estilo card glassmorphism com ícones dinâmicos (sucesso, aviso, erro).
- **Amarrações e Travas Estritas de Data de Eventos**: Bloqueio duplo (interface cliente e backend PHP em [Controllers/EscalaController.php](file:///home/luis/dev/projetos/benApp/Controllers/EscalaController.php)) impedindo o agendamento em datas retroativas (anteriores a hoje) ou em datas que já possuem outro evento cadastrado na célula. Na UI ([Views/Escala/create.php](file:///home/luis/dev/projetos/benApp/Views/Escala/create.php) e [Views/Escala/edit.php](file:///home/luis/dev/projetos/benApp/Views/Escala/edit.php)), o atributo `min` restringe a seleção no calendário, exibe aviso e desabilita automaticamente o botão de salvar.
