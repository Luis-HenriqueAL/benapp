...
<div class="max-w-2xl mx-auto pb-32 space-y-5">
    ...
    <!-- Header Fixo -->
    <div class="sticky top-0 z-40 bg-white/95 backdrop-blur-sm rounded-b-3xl border-b border-slate-100 shadow-lg shadow-slate-200/30 px-5 py-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <!-- Voltar + Título -->
            <div class="flex items-center gap-3">
                <a href="javascript:history.back()" class="p-2 rounded-2xl bg-slate-50 hover:bg-purple-50 text-slate-600 hover:text-purple-600 border border-slate-100 transition-all active:scale-90 flex items-center justify-center shrink-0">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <div class="flex items-center gap-1 bg-purple-50 p-1 rounded-2xl border border-purple-100">
                    <button id="btn-modo-cifra" type="button" onclick="setModoExibicao('cifra')" class="px-3 py-1.5 rounded-xl bg-purple-600 text-white font-extrabold text-xs shadow-xs transition active:scale-95">
                        🎼 Cifra
                    </button>
                    <button id="btn-modo-letra" type="button" onclick="setModoExibicao('letra')" class="px-3 py-1.5 rounded-xl bg-transparent text-purple-700 hover:bg-purple-100 font-bold text-xs transition active:scale-95">
                        📝 Letra
                    </button>
                </div>
                ...
                <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-200">
                    <button type="button" onclick="alterarFonte(-1)" class="w-7 h-7 rounded-xl bg-white hover:bg-slate-50 text-slate-700 font-black text-xs shadow-xs transition active:scale-90 flex items-center justify-center" title="Diminuir fonte">A-</button>
                    <button type="button" onclick="alterarFonte(1)" class="w-7 h-7 rounded-xl bg-white hover:bg-slate-50 text-slate-700 font-black text-xs shadow-xs transition active:scale-90 flex items-center justify-center" title="Aumentar fonte">A+</button>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-slate-100">
            ...
            <div class="flex items-center gap-2 flex-1 min-w-0">
                <button id="btn-autoroll" type="button" onclick="toggleAutoScroll()" class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 border border-slate-200 font-bold text-xs transition active:scale-95 shrink-0">
                ...
                <input id="scroll-speed" type="range" min="1" max="10" value="4" oninput="updateScrollSpeed(this.value)" class="flex-1 min-w-0 accent-purple-600 h-1.5 cursor-pointer rounded-full" title="Velocidade de rolagem">
                <span id="scroll-speed-label" class="text-[10px] text-slate-400 font-bold whitespace-nowrap shrink-0">4x</span>
            </div>

            <!-- Próxima Música -->
            <?php if (count($musicas ?? []) > 1): ?>
                <button type="button" onclick="irParaProximaMusica()" id="btn-proxima" class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 font-extrabold text-xs transition active:scale-95 shrink-0">
                    Próxima
    ...
    <?php if (empty($musicas)): ?>
        <div class="bg-white rounded-3xl p-10 text-center space-y-3 border border-slate-100 shadow-sm">
            <div class="w-14 h-14 bg-purple-50 text-purple-600 rounded-3xl flex items-center justify-center mx-auto text-2xl">🎵</div>
            ...
            <a href="javascript:history.back()" class="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xs rounded-2xl shadow-md shadow-purple-500/20 transition active:scale-95 mt-2">
    ...
    <?php else: ?>
        <div class="space-y-6" id="lista-musicas">
            <?php foreach ($musicas as $index => $m): ?>
                ...
                ?>
                <div id="musica-card-<?= $musicaId ?>" class="musica-card bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden space-y-4 p-5">
                    ...
                    <div class="flex items-start justify-between gap-3 border-b border-slate-100 pb-4">
                        ...
                        <?php if (!empty($m['cifraclub_url'])): ?>
                            <a href="<?= SecurityHelper::e($m['cifraclub_url']) ?>" target="_blank" rel="noopener noreferrer" class="text-[10px] font-bold text-purple-600 hover:text-purple-800 flex items-center gap-1 hover:underline shrink-0 mt-1">
                                Cifra Club
                    ...
                    <?php if (!empty($m['cifra_texto'])): ?>
                        ...
                        <div id="painel-tom-<?= $musicaId ?>" class="bg-amber-50 border border-amber-200/80 p-3 rounded-2xl flex flex-wrap items-center justify-between gap-2">
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-extrabold text-amber-900">Tom:</span>
                                <span id="tom-display-<?= $musicaId ?>" class="px-2.5 py-1 rounded-xl bg-amber-600 text-white font-black text-xs tracking-wide" data-original-tom="<?= $tomOriginal ?>">
                                    <?= $tomOriginal ?>
                                </span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <button type="button" onclick="transporMusica(<?= $musicaId ?>, -1)" class="px-3 py-1.5 rounded-xl bg-white hover:bg-amber-50 border border-amber-300 text-amber-900 font-extrabold text-xs active:scale-95 transition">-1 Semi</button>
                                <button type="button" onclick="transporMusica(<?= $musicaId ?>, 1)"  class="px-3 py-1.5 rounded-xl bg-white hover:bg-amber-50 border border-amber-300 text-amber-900 font-extrabold text-xs active:scale-95 transition">+1 Semi</button>
                                <button type="button" onclick="resetarTomMusica(<?= $musicaId ?>)" class="px-2.5 py-1.5 rounded-xl bg-amber-100 hover:bg-amber-200 text-amber-900 font-bold text-[10px] active:scale-95 transition">Resetar</button>
                            </div>
                        </div>

                        <!-- Bloco da Cifra -->
                        <div class="cifra-wrapper rounded-2xl border border-slate-200 overflow-hidden">
                            <pre id="cifra-pre-<?= $musicaId ?>"
                                 data-original-text="<?= SecurityHelper::e($m['cifra_texto']) ?>"
